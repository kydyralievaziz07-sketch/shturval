...
# Ставится один раз за запрос в общем /api/-шлюзе (do_GET/do_POST), сразу после входа.
_REQ = threading.local()
def _req_company():
    return getattr(_REQ, "company", None) or COMPANY_ID
...
def _my_ig_accounts():
    """Аккаунты ТОЛЬКО компании текущего залогиненного — то, что реально можно отдавать
    в ответ на API-запрос (в отличие от ig_accounts(), которая нарочно нишевая, для
    маршрутизации входящих вебхуков по всей нише)."""
    co = _req_company()
    return [a for a in ig_accounts() if a.get("company_id") == co]

def _ig_account_owned(account_id):
    """Прежде чем отправить что-то ОТ имени account_id (пришёл прямо в теле запроса от
    браузера) — проверяем, что этот аккаунт правда принадлежит компании залогиненного.
    Без этого внутри ниши компания А могла бы писать клиентам компании Б её же токеном."""
    return (ig_account_map().get(str(account_id)) or {}).get("company_id") == _req_company()
...
def ig_rows(limit=1000):
    ...
    try:
        return _supa("GET", "ig_inbox",
                     "?company_id=eq.%s&select=*&order=ts.desc,id.desc&limit=%d" % (_q(_req_company()), limit)) or []
    except Exception:
        return []

def _convos_view(view):
    """Список диалогов из SQL-вью: по одной ПОСЛЕДНЕЙ записи на клиента, ВСЕ диалоги —
    только компании ТЕКУЩЕГО залогиненного (не всего процесса — важно внутри ниши).
    ...
    try:
        ...
        while True:                                   # PostgREST отдаёт максимум 1000 строк — берём страницами
            page = _supa("GET", view,
                         "?company_id=eq.%s&order=ts.desc&limit=1000&offset=%d" % (_q(co), off)) or []
            out.extend(page)
...
def ig_reply(account_id, customer_id, text, by="human"):
    ...
    _ig_mark_mid(mid)
    _co = (ig_account_map().get(str(account_id)) or {}).get("company_id") or COMPANY_ID
    try:
        _supa("POST", "ig_inbox", "",
              {"company_id": _co, "sender_id": str(account_id),
...
def igbot_handle(sender, recipient, text):
    ...
    try:
        ...
        try:
            ...
            return
        _igbot_bump("replies")
        _co = (ig_account_map().get(str(recipient)) or {}).get("company_id") or COMPANY_ID
        try:
            _supa("POST", "ig_inbox", "",
                  {"company_id": _co, "sender_id": str(recipient), "recipient_id": str(sender),
...
def wa_accounts():
    ...
    if not rows and CFG.get("WA_PHONE_ID"):
        rows = [{"phone_id": CFG.get("WA_PHONE_ID"),
                 ...
                 "token": CFG.get("WA_TOKEN", ""), "company_id": COMPANY_ID}]
    return rows

def _my_wa_accounts():
    """Номера ТОЛЬКО компании текущего залогиненного — см. _my_ig_accounts()."""
    co = _req_company()
    return [a for a in wa_accounts() if a.get("company_id") == co]

def _wa_account_owned(phone_id):
    """См. _ig_account_owned() — то же самое для WhatsApp-номеров."""
    return (wa_account_map().get(str(phone_id)) or {}).get("company_id") == _req_company()
...
def wa_reply_media(account_id, customer_id, mtype, media_id, caption="", filename="", mime="", by="human"):
    ...
    _co = (wa_account_map().get(str(account_id)) or {}).get("company_id") or COMPANY_ID
    try:
        _supa("POST", "wa_inbox", "",
              {"company_id": _co, "sender_id": str(account_id),
...
# ===== Рассылка в WhatsApp =====
def _wa_eligible_targets(account=None, max_age_hours=24):
    """Диалоги WhatsApp в окне 24 часов — им можно писать свободным текстом."""
    now = time.time(); cutoff = now - max_age_hours * 3600
    out = []
    for c in wa_conversations():
        ts = c.get("last_ts") or 0
...
def wa_broadcast(text, account=None, limit=500, template="", lang="ru", min_silent_days=0, only=None):
    ...
    for c in targets:
        try:
            if template:
                wa_reply_template(c["account_id"], c["customer_id"], template, lang=lang,
                                  preview=(text or ("\U0001F4CB шаблон: " + template)))
            else:
                wa_reply(c["account_id"], c["customer_id"], text)
            sent += 1
            time.sleep(0.5)
        except Exception as e:
            failed += 1
...
def wabot_handle(sender, phone_id, text=None, fallback=False):
    ...
    try:
        ...
        try:
            ...
            return
        _igbot_bump("replies")
        _co = (wa_account_map().get(str(phone_id)) or {}).get("company_id") or COMPANY_ID
        try:
            _supa("POST", "wa_inbox", "",
                  {"company_id": _co, "sender_id": str(phone_id), "recipient_id": str(sender),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path.startswith("/api/ig/reply"):
            ...
            if not _ig_account_owned(acc):
                return self._send(403, {"error": "Этот аккаунт Instagram вам не принадлежит"})
            try:
        ...
        if self.path.startswith("/api/wa/botreply"):
            ...
            if not _wa_account_owned(acc):
                return self._send(403, {"error": "Этот номер WhatsApp вам не принадлежит"})
            try:
        ...
        if self.path.startswith("/api/wa/read"):
            ...
            try:
                rows = _supa("GET", "wa_inbox",
                             ...
                             % (_q(_req_company()), _q(cust), _q(acc)))
        ...
        if self.path.startswith("/api/suppliers"):
            ...
            try:
                if action == "add_supplier":
                    ...
                    # ВЫПЛАТА долга поставщику НЕ пишется в расходы:
                    # деньги поставщикам — это погашение долга (движение по кредиторке),
                    # а не операционный расход. Учёт ведётся только в модуле «Поставщики»
                    # (таблица supplier_txns). В expenses такие суммы не дублируем.
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/api/wa/media"):
            ...
            if phone and not _wa_account_owned(phone):
                return self._send(403, {"error": "Этот номер WhatsApp вам не принадлежит"})
            try:
        ...
        if self.path.startswith("/api/ig/media"):
            ...
            if acc and not _ig_account_owned(acc):
                return self._send(403, {"error": "Этот аккаунт Instagram вам не принадлежит"})
            try:
