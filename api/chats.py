# -*- coding: utf-8 -*-
"""Облачный эндпоинт /api/chats для Vercel — список диалогов из ChatPlace (Instagram).
Самодостаточный, только стандартная библиотека. Требует env: SITE_PASSWORD, CHATPLACE_KEY."""
import os, json, time, urllib.request
from http.server import BaseHTTPRequestHandler


def chatplace_call(name, arguments):
    key = os.environ.get("CHATPLACE_KEY", "").strip()
    body = json.dumps({"jsonrpc": "2.0", "id": 1, "method": "tools/call",
                       "params": {"name": name, "arguments": arguments}}).encode("utf-8")
    req = urllib.request.Request("https://mcp.chatplace.io/mcp", data=body, headers={
        "Authorization": "Bearer " + key,
        "Content-Type": "application/json",
        "Accept": "application/json, text/event-stream",
        "User-Agent": "Shturval/1.0",
    })
    with urllib.request.urlopen(req, timeout=25) as r:
        resp = json.loads(r.read().decode("utf-8"))
    try:
        return json.loads(resp["result"]["content"][0]["text"])
    except Exception:
        return resp.get("result", {})


def build_chats():
    res = chatplace_call("chats_list", {"limit": 15})
    items = res.get("items", []) if isinstance(res, dict) else []
    chats = []
    for it in items:
        ts = it.get("lastMessageAt")
        tm = time.strftime("%H:%M", time.localtime(ts)) if ts else ""
        chats.append({"id": it.get("id"), "name": it.get("clientName", "клиент"),
                      "time": tm, "status": it.get("statusName", "")})
    return {"source": "ChatPlace · Instagram", "updated": time.strftime("%H:%M:%S"), "chats": chats}


class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        site_pw = os.environ.get("SITE_PASSWORD", "").strip()
        if site_pw and (self.headers.get("X-Auth") or "").strip() != site_pw:
            return self._send(401, {"error": "Требуется вход"})
        if not os.environ.get("CHATPLACE_KEY", "").strip():
            return self._send(200, {"chats": [], "error": "Не задан CHATPLACE_KEY в Vercel"})
        try:
            self._send(200, build_chats())
        except Exception as e:
            self._send(200, {"chats": [], "error": "ChatPlace: " + str(e)})

    def _send(self, code, obj):
        body = json.dumps(obj, ensure_ascii=False).encode("utf-8")
        self.send_response(code)
        self.send_header("Content-Type", "application/json; charset=utf-8")
        self.send_header("Access-Control-Allow-Origin", "*")
        self.send_header("Cache-Control", "no-store")
        self.end_headers()
        self.wfile.write(body)
